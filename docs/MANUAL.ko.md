...
임무형지휘의 "제한" 명시가 거버넌스 규칙이 되는 방식입니다("자유" 쪽은 심사 비대상 목표의 기본 통과로만 표현됩니다).

### 4.9 지휘관 의도 정렬 판정 (`maios align`)

워크스페이스에 `intent.json`으로 지휘관 의도를 명세하면, 행동(목표)이
의도에 부합하는지 실행 전에 판정합니다:

```json
{
  "purpose": "후속 부대 진출 여건 보장",
  "end_state": "주교량 확보 및 적 포병 무력화 상태",
  "key_tasks": ["교량 확보", "적 포병 무력화"],
  "constraints": ["민간 지역 포격 금지"],
  "acceptable_risks": ["야간 기동 간 소부대 고립"]
}
```

```text
maios align 1중대가 교량을 확보하고 통로 개척
[verdict] ALIGNED      ← 핵심과업 지원: 교량 확보

maios align 민간 지역 포격 준비
[verdict] CONFLICT     ← 제한사항과 충돌

maios pursue "민간 지역 포격 계획 수립"
[alignment] CONFLICT - 제한사항과 충돌: 민간 지역 포격 금지
[status] PENDING_APPROVAL   ← 충돌 목표는 자동으로 인간 승인 관문 통과 필요
```
...
"모른다"는 정직한 신호입니다 — 최종 판단은 항상 인간의 몫입니다.
...
모든 심사 결과는 수행 일지(pursuits.json)에 남습니다(감사 로그는 현재 세션 한정). 프로그래밍 방식으로 키워드·위험 등급을
...
| 프로젝트가 하위 목표 1개로 돈다 | LLM의 분해 응답이 목록 형식이 아님 → 실모델에서는 정상 동작하도록 설계됨 (단, 실LLM 검증은 미완) |
| 회상이 엉뚱한 것을 가져온다 | 검색은 한글 바이그램 TF-IDF — 질문에 구체적 명사를 넣을수록 정확 |
| 한글 파일이 깨진다 | UTF-8/cp949 외 인코딩 → UTF-8로 재저장 후 흡수 |
| 흡수 명령이 없다 | v1.3 미만 버전 사용 중 → 업그레이드 (ingest는 v1.3.0부터) |
...
**Q. 데이터가 외부로 나가나?**
LLM 호출 시 프롬프트(목표와 회상 기억·교훈·산출물 발췌)가 해당 프로바이더로 전송됩니다.
