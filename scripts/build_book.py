...
Contents page numbers are a two-pass process: build once, run
``scripts/measure_toc_pages.ps1`` (Word COM) to write
``docs/figures/toc_pages.json``, then build again — the second pass prints
the page numbers with dot leaders. Adding the numbers does not reflow the
book (same line count), so the measured pages stay valid.
"""
...
FIGURES = {
    "4.1": (
        "fig1.png",
        "그림 1. MAIOS 계층 아키텍처와 6단계 인지 루프. 학습 단계의 교훈은 점선 경로를 따라 다음 사이클에 환류된다.",
        112,
    ),
    "5.2": (
        "fig2.png",
        "그림 2. 스프린트별 누적 테스트 수(347→506). 분기 커버리지 95% 게이트를 유지한 채 이틀간 18개 지점에서 증가했다.",
        112,
    ),
    "6.2": ("fig3.png", "그림 3. 자기 기록 되먹임 팽창의 순환(적색)과 세 차단 지점(녹색).", 112),
    "6.3": ("fig4.png", "그림 4. 세 결함 수정의 정량 효과 (동일 실코퍼스, 수정 전 → 후).", 88),
    "7.1": ("fig5.png", "그림 5. 온톨로지 접합부 4곳. J3(점선)은 향후 과제.", 112),
    "7.2": ("fig6.png", "그림 6. 의도 간극의 온톨로지 매개(7.2절 실증).", 112),
    "7.4": ("fig7.png", "그림 7. 의도-행동 정렬의 3치 판정 흐름.", 112),
}

TOC_PAGES = FIGS / "toc_pages.json"

# (표시문구, 들여쓰기 레벨, 본문에서 쪽을 찾을 검색어)
TOC_ENTRIES = [
    ("머리말", 0, "이 책은 하나의 질문에서 시작되었다"),
    ("제1부  사용 안내", 0, "설치부터 온톨로지 결합까지"),
    ("MAIOS란 무엇인가 / 설치 / 5분 훑어보기", 1, "MAIOS란 무엇인가"),
    ("명령어 상세 (pursue · project · research · ingest · align · shell)", 1, "명령어 상세"),
    ("워크스페이스 / 거버넌스 / 실전 워크플로우", 1, "워크스페이스의 구조"),
    ("파이썬 API / 문제 해결 / FAQ", 1, "파이썬 API"),
    ("제2부  설계와 실증 (연구논문)", 0, "기억·회상·거버넌스를 갖춘 임무 중심 인지형"),
    ("서론 / 관련 연구 / 설계 원칙 / 아키텍처", 1, "문제 제기"),
    ("구현과 검증 방법론 / 실사용 코퍼스 사례연구", 1, "구현과 검증 방법론"),
    ("온톨로지 결합과 임무형지휘의 소프트웨어적 번역", 1, "접합 설계"),
    ("한계와 향후 과제 / 결론 / 참고문헌", 1, "실모델 미검증"),
    ("부록  산출물과 개발 이력", 0, "대화형 공동개발의 기록"),
]
...
def add_runs(paragraph, text, size=10.5, base_font=BODY_FONT, color=None):
    """Emit text with **bold**, *italic* and `code` inline markup."""
...
def heading(doc, text, level):
    ...
    p = para(doc, text, size=sizes[level], bold=True, font=HEAD_FONT, space_after=8, line=1.3)
...
def code_block(doc, lines):
    for line in lines:
        ...
        set_font(run, size=8, code=True, color=INK2)
...
def figure(doc, filename, caption, width_mm=112):
    path = FIGS / filename
    if not path.exists():
        return
    p = doc.add_paragraph()
...
def convert_markdown(
    doc, md_text, *, skip_first_heading=True, figures=None, skip_code=False, demote=0
):
    figures = figures or {}
...
def part_page(doc, label, title, subtitle):
    ...
    para(
        doc, label, size=13, bold=True, font=HEAD_FONT, color=BLUE, align=WD_ALIGN_PARAGRAPH.CENTER
    )
    para(
        doc,
        title,
...
def build():
    ...
    para(
        doc,
        "설계 · 실증 · 사용법, 그리고 임무형지휘의 소프트웨어적 번역",
        size=11,
        color=INK2,
        align=WD_ALIGN_PARAGRAPH.CENTER,
        space_after=40,
    ...
    para(
        doc,
        "권영환 지음  ·  Claude(Anthropic) 공동 설계·구현",
        size=10.5,
        align=WD_ALIGN_PARAGRAPH.CENTER,
        space_after=2,
    )
    para(doc, "v1 준비판  ·  2026년 7월", size=9.5, color=INK2, align=WD_ALIGN_PARAGRAPH.CENTER)
    ...
    # ---- contents (page numbers from docs/figures/toc_pages.json, pass 2)
    toc_pages: dict[str, int] = {}
    if TOC_PAGES.exists():
        toc_pages = json.loads(TOC_PAGES.read_text(encoding="utf-8"))
    doc.add_page_break()
    heading(doc, "차례", 1)
    for entry, level, search_key in TOC_ENTRIES:
        p = para(
            doc,
            entry,
            ...
            indent=0.5 if level else None,
            space_after=4,
        )
        page = toc_pages.get(search_key)
        if page:
            p.paragraph_format.tab_stops.add_tab_stop(
                Cm(11.4), WD_TAB_ALIGNMENT.RIGHT, WD_TAB_LEADER.DOTS
            )
            run = p.add_run(f"\t{page}")
            set_font(
                run,
                HEAD_FONT if level == 0 else BODY_FONT,
                10.5 if level else 11.5,
                bold=(level == 0),
            )

    # ---- part 1: manual
    part_page(doc, "제 1 부", "사용 안내", "설치부터 온톨로지 결합까지 — MAIOS 사용 설명서")
    ...
    # ---- appendix
    part_page(doc, "부록", "산출물과 개발 이력", "2026-07-09 ~ 07-10, 대화형 공동개발의 기록")
